...
def process(subFolder, outputDirectory, filename):
	...
	if displayImages:
		imp.show()
		WaitForUserDialog("Title", "Adjust threshold for tissue area").show()

	tissueThreshold = imp.getProcessor().getMaxThreshold()
	...
	if displayImages:
		...
		fieldnames = ["Directory", "Image", "Tissue-threshold"]

		summary["Tissue-threshold"] = tissueThreshold
		for chan in channels:
  			v, x = chan
